document.getElementById("donation-btn-1").addEventListener("click",function(event){
    ...
    const amount1 = document.getElementById("amount-1").value.trim()
    if(amount1.length == 0){
    ...
    const amountA = getInputValueByid("amount-1");
    const balanceMain = getElementInnertextByid("main-balance");
    const balance1 = getElementInnertextByid("balance-1");
    ...
    div.innerHTML = ` <h1 class="text-sm lg:text-xl  font-bold">${amount1} Taka is Donate for Flood at Noakhali, Bangladesh </h1>
